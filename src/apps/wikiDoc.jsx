...
import { ChevronRight, Clock } from '../icons/line.jsx'
import { useGame, wikiPage } from '../engine/store.js'

// 사내위키와 사내 드라이브는 같은 문서를 담는다 — 위키는 문서함으로, 드라이브는
// 열어 놓은 파일로 보여줄 뿐이다. 안쪽 마크업을 둘로 나눠 두면 한쪽만 낡는다.
// 겉을 감싸는 쪽이 클래스로 생김새를 바꾼다.
export default function WikiDoc({ site, id }) {
  ...
  const page = canEdit ? wikiPage(scenario, { wikiEdits, day }, id) : site.wiki.pages[id]
  const save = () => {
    if (!line.trim()) return
    editWiki(id, line)
    setWriting(false)
